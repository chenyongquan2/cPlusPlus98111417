...
public:
	...
	void func2(int x, int y)
	{
		auto mylambda1 = [=, &x, &y]()
		{
			x += 100;
			y += 100;
			m_i = 100;//ok的，因为i=包含了this，成员变量也可以是值传递的来用
		};
	}
};
...
*格式：
*auto lambda表达式名字 [捕获列表](参数列表)->返回类型后置{函数体};(注意最后这里有个分号)
...
*	2.7[=,&变量名]  按值捕获所有外部变量，同时按应用捕获&列出的变量（后面只能出现&引用捕获的，不能出现不带引用的，因为i会与默认的捕获方式重叠）。
*	=必须写在开头位置，第一个位置表示默认捕获方式。
