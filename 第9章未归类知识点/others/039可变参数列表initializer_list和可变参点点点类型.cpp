...
void printValue(initializer_list<int>temArr，int val=10)
{
...
//省略号形参(...) 我们需要用到几个宏
//--求出若干个double类型变量的值
double sum01(int num,...)//可变参数...（必须至少带一个普通的参数num）num传来可变参数的数目
{
	//类型定义（宏）
	va_list valist01;//创建一个va_list类型的变量
	double sum = 0;
	va_start(valist01, num);//使valist01指向起始的参数，确定在内存空间的位置(通过普通参数在栈上来寻址)
	for(int i=0;i<num;i++)
	{
		//遍历参数
...
* 而且带...的可变参数函数必须至少要有一个普通参数，我们就可以通过这个普通参数来寻址后续的所有可变参数。
*
*总结：1.至少有一个有效的形参，不能完全是...
*2.  ...形参只能出现在形参列表中最后一个位置，
*3.  第一个参数会进行类型检查，...中的参数不会进行类型检查。
*4.  ...之前的逗号可以省略void funcChar(const char*msg...)
*5.如果有多个普通参数，那么va_start(valist,msg),第二个参数必须绑定...之前的那一个参数(如果是void myfunc(const char *p1,const char *p2...)，va_start要绑定p2)。
