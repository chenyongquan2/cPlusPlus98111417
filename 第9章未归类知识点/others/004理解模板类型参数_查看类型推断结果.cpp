...
{
	//T的类型不仅和调用者提供的实参(100)有关系，还和tem的类型(const T&整个为tem的类型)有关。
	using boost::typeindex::type_id_with_cvr;
	cout << "T type=" << type_id_with_cvr<T>().pretty_name() << endl;//显示T类型
	cout << "tem type=" << type_id_with_cvr<decltype(tem)>().pretty_name() << endl;//显示形参的类型
...
int main(void)
{
	...
	const int&k = i;	//const int&类型 //k为i的别名
	cout << "-----" << endl;
	myFunction02(i);
	myFunction02(j);
	myFunction02(k);
	 ...
	 *	T type=int
		...
		T type=int const
		tem type=int const &
	1.如果实参是引用类型，则其的引用类型属性会被忽略掉，T不会被推到为引用类型。
	2.当我们向引用类型的形参tem传递const类型实参时，形参就会成为const&
		实参的const属性会成为类型模板参数T的推导的类型的组成部分，
		也就是说，实参的const属性会传递。在函数myFunction02中修改不了原来带const属性的实参。
	...
	2.对于有const属性的实参，推导后T中的const属性没有了(因为实参已经有const属性了)，因为模板函数myFunction03的形参tem出现了const。实参const替换掉形参const
	...
	//*	2.2万能引用--形式参数tem是一个万能引用类型T&&，根据你传进来的实参是左值还是右值，万能引用的表现是不同的。
	cout << "2.2------------------------" << endl;
	myFunction05(i);//左值 
	myFunction05(j);//左值
	myFunction05(k);
	...
	myFunction06(j);
	myFunction06(k);
	//void myFunction06(T tem)//因为这里是值拷贝，而不是传引用。
	/*
	 *  T type=int
		tem type=int
		T type=int
		...
		T type=int
		tem type=int
	const属性没有传递进去，因为形式参数是一个新副本。怎么传递进去const属性？？？
	 */
	//-现在我们传递指针作为形参进去
	...
	//总结：如果你传递的是const char*或者const char[] 数组，那么这个const会被保留。
	 */
