...
int main(void)
{
	...
	//第一册是利用ct来产生一个临时对象tmp，第二次是std::bind要返回一个CT类型对象(仿函数)，返回的CT对象拷贝自临时对象tmp，std::bind执行完毕后临时对象tmp会释放。
	//但这个返回的CT对象(上面的那个仿函数)就被弄到了bf2去了
	bf2() = 53;//实际修改的是成员变量的值


	auto rt = std::bind(CT());//执行构造函数，拷贝构造，两次析构
	/*
	 * CT()构造临时对象tmp，调用拷贝构造函数生成一个临时对象作为std::bind()的返回对象(仿函数)，
	 * bind返回上面的仿函数类型对象，就是这个调用拷贝构造函数构造出来的对象
	 */
