...
int main(void)
{
	...
	*p2 = 43;
	...
	decltype(i) k3 = j;//k3=int,i只是个变量
	//在变量名字外面多加一层括号，编译器会把（变量）当作一个表达式
	//又因为 (变量)可以作为等号左边的内容。
	decltype((i))reference03 = j;//最终结果是个引用
	//结论：decltype((变量))的结果永远是个引用

	//1.3decltype后的圆括号是个函数
	decltype(testFunction())temv = 14;//temv是函数的返回值类型，但是编译器不会真正调用函数,只是使用了函数testf()的返回值类型作为temv的类型
...
*		decltype说明符：主要作用：返回操作数的数据类型(因为是啥就返回啥，除了一些特殊情况之外。)
*
*		decltype的特点：
*			1.decltype的自动类型推断会发生在编译期间(和auto一样)
