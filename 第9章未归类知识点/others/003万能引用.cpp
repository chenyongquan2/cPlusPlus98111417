...
//void myFunction03(const T1 &&tem)//有const修饰，不可能是万能引用。 
//void myFunction02(int&& tem)
void myFunction03(T1 &&tem)//&&属于tem形参类型的一部分，&&和T类型没有关系
{
	//T&&或者tem才是万能引用，而单独的T不是万能引用。
	//如果传进来一个整形的左值进来，tem它就应该被推断为int&类型
	//如果传进来一个整形的右值进来，tem它就应该被推断为int&&类型
	cout << "右值引用模板函数" << endl;
	return;
}
...
{
public:
	void testFunction(T&& tem)//这个是右值引用，(因为要调用这个成员函数之前，才是先实例化把T2的类型给定下来了,类模板实例化后了。没有涉及到类型推断)
	{
		
	}

	template<typename T3>
	void testFunction02(T3&& tem)//这个才是万能引用
	{
 ...
 *		  {}//右值引用，模板类型T，必须和&&挨着，才是一个右值引用
 *
 *(5)万能引用资格的剥夺和辨认
 *	1.剥夺：const会剥夺成为右值引用的资格，使其成为右值引用  void myFunction03(const T1 &&tem)//这里是右值引用
 *	2.辨认：类的成员函数辨认
 *	2019年12月9日11点23分
 */
