...
int main(void)
{
	...
	myFunction01(number03);//T=int const tem=const int& number03的引用属性会被忽略，传递不过去
	/*
	T type=int
	tem type=int &
	T type=int const
	...
	auto&&number13 = number12;//auto=const int&，number13=const int&
...
*2.auto定义的变量必须立即初始化，这样编译器才能自动推断它的类型。确定auto的类型和它所定义变量的类型。(auto也是有类型的，类似函数模板中的T，也是有类型的)
