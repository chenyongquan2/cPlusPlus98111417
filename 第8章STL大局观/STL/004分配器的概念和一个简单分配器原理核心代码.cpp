...
50
004BD728
	总结：经过测试，内存不连续，底层不是使用内存池技术实现，估计是直接调用malloc()函数(只能说明该测试是，不代表其他的编译器也是如此)
	 */
...
*内存池方法就是分配一大块内存，每次从中取出一下快满足要求的内存。避免每次都malloc ，减少浪费内存。
*而分配器，就是扮演内存池的角色
*
...
*那么可以设置一个全局的分配器(见图，数字代表尺寸(8的倍数)，)，每个分配器设置若干个内存池，供各个容器共同使用这个分配器。详细见图。
*
*(5)自定义分配器
*	代码繁琐，并不好写
*
