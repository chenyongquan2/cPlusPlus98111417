...
*	1.static_cast
*		静态转换理解为“正常转换”，"编译"的时候进行类型转换的检查。代码中要保证转换的安全性，如“asdf”字符串转换为int是错误的
*		与c语言的强制类型转换差不多。
...
*	2.dynamic_cast
*		主要应用于"运行时"(只有这个是运行时转换的)类型识别和检查
*		应用于"父类型和子类型之间转换"---父类型指针指向子类对象（父类指针用子类对象初始化）,然后用dynamic_cast把父类指针转换为子类
*	3.const_cast
*		去除指针或者引用的const属性，也是属于"编译时"的转换。
*		不能去掉普通变量的。
*	4.reinterpret_cast
*		重新解释转换，将操作数重新解释为另一种类型--把操作数类型都转换了
*		处理“无关类型”的转换--两个转换之间没有关系。自由转换，非常随意。
