...
public:
	...
	{
		//return shared_ptr<CT>(this);//直接裸指针初始化多个智能指针？
...
int main(void)
{
	//(1)
	myFunction(100);
	shared_ptr<int>p1 = myFunction(10);//如果不用变量来接受，返回的局部变量就会销毁。接管了这个临时对象。
	...
	*p = 22;//这一段内存已经被回收，临时对象传递给函数会是个局部量，函数运行完毕会释放。
	...
	//不要使用裸指针初始化多个shared_ptr--会导致内存释放多次 p3,p4这两个智能指针无关(不共享引用计数/控制块)
	/*int*p2 = new int(200);
	shared_ptr<int>p3(p2);
	shared_ptr<int>p4(p2);*/
	//因此建议不要把非匿名的裸指针直接赋值给智能指针。
	//更改方法：
	...
	//shared_ptr<CT>Pct2(Pct1);//两个强引用,使用同一个控制块，ok的
	shared_ptr<CT>pct3 = Pct1->getself();//两个智能指针之间不是使用同一个控制块，会导致Ct对象被释放两次。有异常。
	//没有关系.有一种使用裸指针初始化智能指针的
	 ...
	 *	只需要将其中一个变为弱引用即可。
	 */
	pca->pm_b = pcb;//等价于指向pb的对象有两个强引用。
	pcb->pm_a = pca;//因为pm_a是弱引用，所以这里指向ca的对象只有一个强引用，离开作用域之后，pa的引用计数从1变为0就会释放ca对象
	//(Ca的析构函数被执行了，导致CA内的智能指针pm_b引用计数减一，也就是指向Cb对象的引用计数-1)
	//超出cb的作用域后，cb的引用计数也会-1，最终会有一个时刻,指向CB对象的强引用计数会从1减少到0，导致cb对象也被释放。

	//3.2移动语义
	shared_ptr<int>p7(new int(1000));
	shared_ptr<int>p8(std::move(p1));//移动构造语义，移动构造一个新的智能指针对象p8,p7不再指向1000，
	shared_ptr<int> p88;
	p88=std::move(p8);//移动赋值语义
	//引用计数还是1
...
*	由第一个指向某个指定对象的shared_ptr来 创建，后面基于此shared_ptr也基于这个控制块。
*	创建控制块的时机：
*	1.make_shared,分配并初始化一个对象，返回指向此对象的shared_ptr,这货总是能创建一个控制块的。
*	2.用裸指针来创建爱女一个shared_ptr对象时候，使用同一个裸指针分别创建shared_ptr会产生多个控制块，详见2.2
*	
*3.2移动语义
...
*	3.优先使用make_shared，编译器有优化这个函数。(虽然这货不能定义自个的删除器！)
*
*/
