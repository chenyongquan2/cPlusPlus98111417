...
int main(void)
{
	...
	weak_ptr<int>p2Weak(p1);//p2Weak是弱引用，强引用计数不会改变，但是弱引用计数会改变。一个强引用，一个弱引用指向100
	...
	//常见函数用法
	int count = p2Weak.use_count();//拿强引用计数数目
	cout << count << endl;	//2

	//expired--过期判断，判断该指针指向对象的use_count是否为0(表示该弱指针指向的对象已经不存在了)
	//指向100的两个强引用都设置为过期
	p1.reset();
	p4shared_ptr.reset();
...
*换句话说，将weak_ptr绑定到shared_ptr上不会改变shared_ptr对象的引用计数。(更确切的说，weak_ptr的构造和析构不会增加或者减少所指向对象的引用计数)
