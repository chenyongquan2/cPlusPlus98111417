...
inline //模板函数可以是内联的，inline的位置放在模板函数列之后
...
int main(void)
{
	...
	int a = 12;
	//int Sum02 = FunctionTemplate02<a, 32>();//error，必须用常量，编译时候就能确定的
...
*		但是这些值都得是"常量表达式"，因为实例化这些模板都是在编译的时候确定的（而不是运行时）。
*
*(3)模板定义并不会生成代码，只有在我们调用函数模板时候，编译器为我们实例化一个特定的版本函数后才会生成代码。
*	编译器生成代码的时候，需要找到函数的函数体。所以函数模板的定义通常都是在.h中。（方便include的时候把生成的函数体定义函数体实现都给include进来了）
*	//这种把函数体实现和定义都放在一个头文件叫template.h里，假设若有pro1.cpp和pro2.cpp都include<template.h>，也不会报错重定义，而如果是普通函数的实现放在.h里，则会报错。
*	
*
*/
