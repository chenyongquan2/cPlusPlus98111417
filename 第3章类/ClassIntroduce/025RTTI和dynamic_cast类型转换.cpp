...
int main(void)
{
	...
	//也可typeid(类型名字)
	if(typeid(*p_human03)==typeid(Woman))
	{
 ...
 *		(这样这个类才会产生一个虚函数表，当对象创建的时候会给对象产生一个虚函数表指针指向虚函数表，而虚函数表的第一项存放的就是这个类的type_info()对象，
 *		这个对象保存着类的类型相关信息)
 *		否则，编译的时候就确定了调用哪个类型。
 ...
 * 虚函数表第一项是type_info()类型，用于说明实际的类对象类型。调用子类或者父类的函数（虚函数）
 */
