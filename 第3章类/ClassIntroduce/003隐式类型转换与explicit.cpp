...
int main(void)
{
	Time myTime01 = 12;//Todo:编译系统有个行为，将12这个数字转化成了一个Time类型，存在了临时对象隐式转换问题。
	//Time myTime03(12, 23, 43, 54);	//错误
	Time myTime04 = { 16 };
	//大括号内部可以当做一个对象·	--如果带一个参数的构造函数加explicit就会进行隐式类型转换，都会错误
	Time myTime02=(12, 23, 43, 54);	//对()内部发生了隐式类型转换，把()转化为对象， “调用了单参数的构造函数”--相当于逗号表达式，最后是Hour=54

	functionTransform(12);	//首先会调用一个参数的构造，构造一个Time临时对象，存在了临时对象隐式转换问题。
