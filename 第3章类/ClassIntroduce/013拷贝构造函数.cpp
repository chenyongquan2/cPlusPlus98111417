...
public:
	...
	//Time(Time&myTime, int TemHour = 10);//拷贝构造函数只能有一个
};
...
*	4.成员变量逐个拷贝因为我们定义的拷贝构造函数存在而失去作用。(系统合成的拷贝构造函数有)
*		我们定义的拷贝构造函数取代了系统默认的拷贝构造函数。所以我们得自个写。
*(3)
...
*	4.如果自己定义了拷贝构造函数，就能会取代系统的拷贝构造函数，那么就需要你自己在拷贝构造函数中给类成员赋值了 。
*		以防止出现未初始化的成员变量。
