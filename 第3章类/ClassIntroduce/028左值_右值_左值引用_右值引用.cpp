...
int main(void)
{
	...
	//(number11++) = 2000;//error
	int&&reference13 = number11++;//成功绑定右值，但是reference11绑定的不是number11,绑定的是返回的临时量，绑定的和number11再无关系
	
	{
		//终点强调
		int &&r1 = 10;
		int &r2 = r1;//ok,r1虽然是个右值引用（绑定到右值)，但是本身它是一个左值
	}

	//std::move()函数--左值转换为右值
	int number14 = 14;
	int&&reference_number15 = std::move(number14);//ok
	reference_number15 = 15;//这时候number14也跟着变了。
	...
	string str18 = "jisuanjizuchegnaun";//没有移动，触发了string类的移动构造函数。移动后把str18清空了。
	//reference19是重新开辟的内存(然后把"jisuanjizuchegnaun"给拷贝过去)，没有节省啥内存。
	string reference19 = std::move(str18);//移动构造函,执行完这句话后str18变成了"",而reference19变成了 "jisuanjizuchegnaun"
	string &&str20 = std::move(str18);//没有执行移动构造函，而是把一段右值绑定到右值上
...
*(1)引用分类
*	1.左值引用-绑定到左值
*	2.常量引用-不改变原来的值,也是左值引用。
*	3.右值引用 绑定到右值 使用&& c++11标准
*(2)左值引用是什么？
*	引用左右，绑定到左值上
*
*(3)右值引用，必须绑定到右值上
...
*2.任何函数的形参都是左值。如void function(int&&a);a虽然是右值引用(绑定到右值)，但是本身是左值
*3.临时对象是右值。
*
*
*(4)右值引入目的
...
*		当用一个对象给另一个对象赋值，
		1）移动构造函数/移动赋值运算符（需要的参数是右值引用）
*		2）拷贝构造函数/赋值运算符需要的参数是左值引用
*
*(5)std::move()函数(move移动的意思，但是其实这个函数根本没有做移动的操作)"思想:std::move(a) 承诺不再使用里面a的值了"
